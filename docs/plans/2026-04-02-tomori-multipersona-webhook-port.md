...
- Verified current repo structure for this task is the live workspace at `E:\femboibot`, where the reusable streaming stack already exists under `discord_bot/utils/streaming/` and the focused stream tests already exist under `tests/test_stream_*.py`.
- Clean committed `HEAD` does not yet contain those files, so execution in the isolated worktree begins by syncing the existing task-scoped streaming modules and related tests from the verified workspace.
- All task steps and verification commands below are executed against that verified current workspace structure, not the older clean `HEAD` alone.

...
- Agentic action execution (role management, channel/category creation, and moderation) via structured JSON responses when user has configured staff permissions; role creation can omit a target user and can auto-assign to the requester when asked; supports unban; includes fallback parsers for owner/admin role and channel requests; logs to mod-log channel.

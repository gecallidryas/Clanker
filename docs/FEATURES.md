...
- Agentic action execution (role management and moderation) via structured JSON responses when user has configured staff permissions; role creation can omit a target user and can auto-assign to the requester when asked; supports unban; includes a fallback parser for owner/admin role requests; logs to mod-log channel.

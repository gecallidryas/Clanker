...
class ReplySequenceModal(discord.ui.Modal):
    def __init__(self, *, guild_id: int) -> None:
        super().__init__(title="Reply Sequence")
        self.guild_id = guild_id
        self.timeout_seconds = discord.ui.TextInput(label="Timeout seconds", required=False, placeholder="30-3600")
        self.max_stages = discord.ui.TextInput(label="Max stages", required=False, placeholder="1-10")
        self.add_item(self.timeout_seconds)
        self.add_item(self.max_stages)

    async def on_submit(self, interaction: discord.Interaction) -> None:
        updates: dict[str, Any] = {}
        if self.timeout_seconds.value.strip():
            seconds = int(self.timeout_seconds.value.strip())
            if seconds < 30 or seconds > 3600:
                await interaction.response.send_message("Timeout must be between 30 and 3600.", ephemeral=True)
                return
            updates["reply_sequence_timeout_seconds"] = seconds
        if self.max_stages.value.strip():
            stages = int(self.max_stages.value.strip())
            if stages < 1 or stages > 10:
                await interaction.response.send_message("Max stages must be between 1 and 10.", ephemeral=True)
                return
            updates["reply_sequence_hard_max_stages"] = stages
        if not updates:
            await interaction.response.send_message("No reply-sequence changes submitted.", ephemeral=True)
            return
        await update_guild_config(self.guild_id, updates)
        await add_guild_config_audit(
            self.guild_id,
            interaction.user.id,
            "reply_sequence_update",
            category=AUDIT_CATEGORY_CONFIG_GENERAL,
            summary="Updated reply-sequence limits.",
            detail=updates,
        )
        await interaction.response.send_message("Reply-sequence settings updated.", ephemeral=True)
...
class AISettingsHomeView(AdminPanelView):
    ...
    async def build_embed(self) -> discord.Embed:
        ...
        embed.add_field(name="Reply sequences", value="Enabled" if config.get("reply_sequence_enabled") else "Disabled", inline=True)
    ...
    @discord.ui.button(label="Reply Sequence", style=discord.ButtonStyle.primary, row=0)
    async def reply_sequence_button(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        if not await _ensure_low_risk_permissions(interaction):
            return
        view = ReplySequenceView(guild_id=self.guild_id, user_id=self.user_id, locale=self.locale, home_view=self)
        await _swap_panel(interaction, embed=await view.build_embed(), view=view, previous=self)
...
class ReplySequenceView(AdminPanelView):
    ...
    async def build_embed(self) -> discord.Embed:
        config = await get_guild_config(self.guild_id)
        embed = discord.Embed(title="Reply Sequence", color=discord.Color.blue())
        embed.add_field(name="Enabled", value="Yes" if config.get("reply_sequence_enabled") else "No", inline=True)
        embed.add_field(name="Timeout", value=f"{config.get('reply_sequence_timeout_seconds') or 300}s", inline=True)
        embed.add_field(name="Max stages", value=str(config.get("reply_sequence_hard_max_stages") or 4), inline=True)
        embed.add_field(name="GIF payloads", value="Yes" if config.get("reply_sequence_allow_gif", 1) else "No", inline=True)
        embed.add_field(name="Sticker payloads", value="Yes" if config.get("reply_sequence_allow_sticker", 1) else "No", inline=True)
        embed.add_field(name="Emoji-only payloads", value="Yes" if config.get("reply_sequence_allow_emoji_only", 1) else "No", inline=True)
        return embed

    async def _toggle_flag(self, interaction: discord.Interaction, field: str, action: str) -> None:
        config = await get_guild_config(self.guild_id)
        current = bool(config.get(field) or 0)
        updates = {field: 0 if current else 1}
        await update_guild_config(self.guild_id, updates)
        await add_guild_config_audit(
            self.guild_id,
            interaction.user.id,
    ...
    async def toggle_enabled(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await self._toggle_flag(interaction, "reply_sequence_enabled", "reply_sequence_enabled_toggle")

    @discord.ui.button(label="Toggle GIF", style=discord.ButtonStyle.secondary, row=1)
    async def toggle_gif(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await self._toggle_flag(interaction, "reply_sequence_allow_gif", "reply_sequence_allow_gif_toggle")

    @discord.ui.button(label="Toggle Sticker", style=discord.ButtonStyle.secondary, row=1)
    async def toggle_sticker(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await self._toggle_flag(interaction, "reply_sequence_allow_sticker", "reply_sequence_allow_sticker_toggle")

    @discord.ui.button(label="Toggle Emoji-Only", style=discord.ButtonStyle.secondary, row=1)
    async def toggle_emoji(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await self._toggle_flag(interaction, "reply_sequence_allow_emoji_only", "reply_sequence_allow_emoji_toggle")

    @discord.ui.button(label="Edit Limits", style=discord.ButtonStyle.primary, row=2)
    async def edit_limits(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await interaction.response.send_modal(ReplySequenceModal(guild_id=self.guild_id))

...
from cogs.ai_brain import AIBrain, StreamEvent, _is_processing_ack_response  # noqa: E402
...
def test_generate_response_stream_rejects_native_streaming_tool_calls():
    async def _run():
        ...
        with mock.patch("cogs.ai_brain.get_evil_mode", new=mock.AsyncMock(return_value=True)):
            with mock.patch("cogs.ai_brain.stream_guild_openrouter_text", new=fake_stream):
                events = []
                async for event in AIBrain.generate_response_stream(
                    brain,
                    "prompt",
                    123,
                    allow_evil=True,

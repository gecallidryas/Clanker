...
async def _create_guild_config_table(db: aiosqlite.Connection) -> None:
    await db.execute(
        """
        CREATE TABLE IF NOT EXISTS guild_config (
            ...
            normal_text_provider TEXT DEFAULT 'gemini',
...
async def _rebuild_guild_config_without_removed_columns(db: aiosqlite.Connection) -> None:
    ...
    if not any(column in existing_columns for column in REMOVED_REPLY_SEQUENCE_FIELDS):
        return

    keep_columns = [column for column in existing_columns if column not in REMOVED_REPLY_SEQUENCE_FIELDS]
    select_list = ", ".join(keep_columns)

    await db.execute("ALTER TABLE guild_config RENAME TO guild_config_legacy_reply_sequence")
    await _create_guild_config_table(db)
    await db.execute(
        f"INSERT INTO guild_config ({select_list}) SELECT {select_list} FROM guild_config_legacy_reply_sequence"
    )
    await db.execute("DROP TABLE guild_config_legacy_reply_sequence")
...
async def _init_guild_schema(db: aiosqlite.Connection) -> None:
    ...
    try:
        ...
        logger.exception("Failed to migrate guild_config schema")
        raise
    ...
    try:
        ...
        for column_name, column_type, default_value in [
            ...
            ("normal_text_provider", "TEXT", "gemini"),
        ...
        await _rebuild_guild_config_without_removed_columns(db)
    except Exception:
        pass
...
async def sanitize_active_persona_modes(guild_id: int, mode_keys: list[str]) -> list[str]:
    ...
    fallback_mode = await get_server_mode(guild_id)
    ...
    async with guild_db(guild_id) as db:
        ...
        async with db.execute(
            ...
            if not row:
                return {}
            config = dict(row)
            allowed = GUILD_CONFIG_FIELDS | {"guild_id", "updated_at"}

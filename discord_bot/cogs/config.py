...
CONFIG_TOGGLE_OPTIONS = [
    ...
    ("reply_sequence_enabled", "Reply sequences"),
    ("reply_sequence_allow_gif", "Sequence GIF"),
    ("reply_sequence_allow_sticker", "Sequence sticker"),
    ("reply_sequence_allow_emoji_only", "Sequence emoji-only"),
...
class Config(commands.Cog):
    ...
    def _build_ai_embed(self, config: dict[str, Any]) -> discord.Embed:
        ...
        embed = discord.Embed(
            title="AI Settings",
            description="Reply policy, routing, streaming, and thought/debug logging.",
            color=discord.Color.blue(),
        )
        embed.add_field(
            name="Reply policy",
    ...
    async def ai_view(self, interaction: discord.Interaction):
        ...
        reply_sequence_enabled = bool(config.get("reply_sequence_enabled") or 0)
        reply_sequence_timeout = int(config.get("reply_sequence_timeout_seconds") or 300)
        reply_sequence_stages = int(config.get("reply_sequence_hard_max_stages") or 4)
        reply_sequence_allow_gif = bool(config.get("reply_sequence_allow_gif", 1))
        reply_sequence_allow_sticker = bool(config.get("reply_sequence_allow_sticker", 1))
        reply_sequence_allow_emoji_only = bool(config.get("reply_sequence_allow_emoji_only", 1))
        ...
        embed.add_field(
            name="Reply sequences",
            value=(
                f"enabled={reply_sequence_enabled}, "
                f"timeout={reply_sequence_timeout}s, "
                f"hard_max_stages={reply_sequence_stages}"
            ),
            inline=False,
        )
        embed.add_field(
            name="Reply-sequence payloads",
            value=(
                f"gif={reply_sequence_allow_gif}, "
                f"sticker={reply_sequence_allow_sticker}, "
                f"emoji_only={reply_sequence_allow_emoji_only}"
            ),
            inline=False,
        )
    ...
    @ai_group.command(name="reply_sequence", description="Enable or disable active-mode reply sequences.")
    @app_commands.checks.has_permissions(administrator=True)
    @app_commands.describe(state="on/off")
    async def ai_set_reply_sequence(self, interaction: discord.Interaction, state: str):
        if not await self._require_guild(interaction):
            return
        if not await self._require_auth(interaction):
            return
        ...
        await update_guild_config(interaction.guild.id, {"reply_sequence_enabled": int(enabled)})
        await add_guild_config_audit(interaction.guild.id, interaction.user.id, "reply_sequence_enabled_set")
        await interaction.response.send_message(
            f"Reply sequences {'enabled' if enabled else 'disabled'}.",
            ephemeral=True,
        )

    @ai_group.command(name="reply_sequence_timeout", description="Set reply-sequence timeout in seconds.")
    @app_commands.checks.has_permissions(administrator=True)
    @app_commands.describe(seconds="Timeout before a pending sequence expires (30-3600)")
    async def ai_set_reply_sequence_timeout(self, interaction: discord.Interaction, seconds: int):
        if not await self._require_guild(interaction):
            return
        if not await self._require_auth(interaction):
            return
        if seconds < 30 or seconds > 3600:
            await interaction.response.send_message("Use a value between 30 and 3600.", ephemeral=True)
            return
        await update_guild_config(interaction.guild.id, {"reply_sequence_timeout_seconds": int(seconds)})
        await add_guild_config_audit(interaction.guild.id, interaction.user.id, "reply_sequence_timeout_seconds_set")
        await interaction.response.send_message(
            f"Reply-sequence timeout set to {seconds}s.",
            ephemeral=True,
        )

    @ai_group.command(name="reply_sequence_stages", description="Set the hard max bot stages in a reply sequence.")
    @app_commands.checks.has_permissions(administrator=True)
    @app_commands.describe(stages="Maximum bot messages in one sequence (1-10)")
    async def ai_set_reply_sequence_stages(self, interaction: discord.Interaction, stages: int):
        if not await self._require_guild(interaction):
            return
        if not await self._require_auth(interaction):
            return
        if stages < 1 or stages > 10:
            await interaction.response.send_message("Use a value between 1 and 10.", ephemeral=True)
            return
        await update_guild_config(interaction.guild.id, {"reply_sequence_hard_max_stages": int(stages)})
        await add_guild_config_audit(interaction.guild.id, interaction.user.id, "reply_sequence_hard_max_stages_set")
        await interaction.response.send_message(
            f"Reply-sequence hard max stages set to {stages}.",
            ephemeral=True,
        )

    async def _set_reply_sequence_payload_flag(
        self,
        interaction: discord.Interaction,
        *,
        field: str,
        ...
        if normalized not in {"on", "off", "enable", "disable", "true", "false"}:
            ...
            return
        enabled = normalized in {"on", "enable", "true"}
        await update_guild_config(interaction.guild.id, {field: int(enabled)})
        await add_guild_config_audit(interaction.guild.id, interaction.user.id, f"{field}_set")
        await interaction.response.send_message(
            f"Reply-sequence {label} {'enabled' if enabled else 'disabled'}.",
            ephemeral=True,
        )

    @ai_group.command(name="reply_sequence_gif", description="Enable or disable GIF payloads in reply sequences.")
    @app_commands.checks.has_permissions(administrator=True)
    @app_commands.describe(state="on/off")
    async def ai_set_reply_sequence_gif(self, interaction: discord.Interaction, state: str):
        await self._set_reply_sequence_payload_flag(
            interaction,
            field="reply_sequence_allow_gif",
            label="GIF payloads",
            state=state,
        )

    @ai_group.command(name="reply_sequence_sticker", description="Enable or disable sticker payloads in reply sequences.")
    @app_commands.checks.has_permissions(administrator=True)
    @app_commands.describe(state="on/off")
    async def ai_set_reply_sequence_sticker(self, interaction: discord.Interaction, state: str):
        await self._set_reply_sequence_payload_flag(
            interaction,
            field="reply_sequence_allow_sticker",
            label="sticker payloads",
            state=state,
        )

    @ai_group.command(name="reply_sequence_emoji", description="Enable or disable emoji-only payloads in reply sequences.")
    @app_commands.checks.has_permissions(administrator=True)
    @app_commands.describe(state="on/off")
    async def ai_set_reply_sequence_emoji(self, interaction: discord.Interaction, state: str):
        await self._set_reply_sequence_payload_flag(
            interaction,
            field="reply_sequence_allow_emoji_only",
            label="emoji-only payloads",
            state=state,
        )
